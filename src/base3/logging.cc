...
// static
LogLockingState LoggingLock::lock_log_file = DONT_LOCK_LOG_FILE;
...
bool InitializeLogFileHandle() {
  ...
  if (!log_file_name_) {
    ...
    log_file_name_ = new PathString(GetDefaultLogFile());
  }
...
bool BaseInitLoggingImpl(const PathChar* new_log_file,
  ...
  if (!log_file_name_)
    log_file_name_ = new PathString();
  *log_file_name_ = new_log_file;
  if (delete_old == DELETE_OLD_LOG_FILE)
    DeleteFilePath(*log_file_name_);
...
LogMessage::~LogMessage() {
  ...
  // we(arch) DONOT need lock for log
  // LoggingLock::Init(DONT_LOCK_LOG_FILE, NULL);
